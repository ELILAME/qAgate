...
void qTdep::tdep()
{
  try
  {
    ...
    _eigparser.reset(EigParser::getEigParser(tdep.outputPrefix()+"phonon-bands.yaml"));
...
void qTdep::replotEigparser()
{
  if (_eigparser == nullptr )
    {
      //QMessageBox::information(this,tr("Missing data"),tr("TDEP did not run properly, so there is no dispersion to plot"));
      return;
    }
  ConfigParser config;
